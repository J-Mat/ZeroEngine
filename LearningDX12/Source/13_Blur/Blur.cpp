...
class FBlur : public FD3DApp
{
public:
	FBlur(HINSTANCE hInstance);
	~FBlur();
...
private:
	...
	std::array<const CD3DX12_STATIC_SAMPLER_DESC, 6> FBlur::GetStaticSamplers();
...
private:
	...
	std::unique_ptr<FBlurFilter> BlurFilter;
...
FBlur::FBlur(HINSTANCE hInstance)
	: FD3DApp(hInstance)
{
}
...
float FBlur::GetHillsHeight(float X, float Z) const
{
	return 0.3f * (Z * sinf(0.1f * X) + X * cosf(0.1f * Z));
}

XMFLOAT3 FBlur::GetHillsNormal(float X, float Z) const
{
	XMFLOAT3 N(
...
bool FBlur::Initialize()
{
	...
	BlurFilter = std::make_unique<FBlurFilter>(D3DDevice.Get(),
...
void FBlur::OnResize()
{
	...
	{
		BlurFilter->OnResize(ClientWidth, ClientHeight);
	}
}

void FBlur::Update(const GameTimer &gt)
{
	OnKeyboardInput(gt);
	UpdateCamera(gt);
...
void FBlur::Draw(const GameTimer &gt)
{
	...
	BlurFilter->Execute(CommandList.Get(), PostProcessRootSignature.Get(),
		PSOs["horzBlur"].Get(), PSOs["vertBlur"].Get(), GetCurrentBackBuffer(), 1);

	// Prepare to copy blurred output to the back buffer.
	CommandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition(GetCurrentBackBuffer(),
		D3D12_RESOURCE_STATE_COPY_SOURCE, D3D12_RESOURCE_STATE_COPY_DEST));

	CommandList->CopyResource(GetCurrentBackBuffer(), BlurFilter->Output());
...
void FBlur::OnMouseDown(WPARAM btnState, int x, int y)
{
	LastMousePos.x = x;
	LastMousePos.y = y;
...
void FBlur::OnMouseUp(WPARAM btnState, int x, int y)
{
	ReleaseCapture();
}

void FBlur::OnMouseMove(WPARAM btnState, int x, int y)
{
...
void FBlur::OnKeyboardInput(const GameTimer &gt)
{
...
void FBlur::UpdateCamera(const GameTimer &gt)
{
...
void FBlur::UpdateObjectCBs(const GameTimer &gt)
{
...
void FBlur::UpdateMaterialCBs(const GameTimer& gt)
{
...
void FBlur::UpdateMainPassCB(const GameTimer &gt)
{
...
void FBlur::UpdateWaves(const GameTimer& gt)
{
...
void FBlur::BuildPostProcessRootSignature()
{
...
void FBlur::BuildDescriptorHeaps()
{
	...
	ThrowIfFailed(D3DDevice->CreateDescriptorHeap(&SrvHeapDesc, IID_PPV_ARGS(&CbvSrvUavDescriptorHeap)));
	...
	BlurFilter->BuildDescriptors(
		CD3DX12_CPU_DESCRIPTOR_HANDLE(CbvSrvUavDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), 3, CbvSrvDescriptorSize),
		CD3DX12_GPU_DESCRIPTOR_HANDLE(CbvSrvUavDescriptorHeap->GetGPUDescriptorHandleForHeapStart(), 3, CbvSrvDescriptorSize),
		CbvSrvDescriptorSize
	);
	
}

void FBlur::BuildShadersAndInputLayout()
{
	...
	Shaders["standardVS"] = d3dUtil::CompileShader(L"Shaders\\13_Blur\\Default.hlsl", nullptr, "VS", "vs_5_0");
	Shaders["opaquePS"] = d3dUtil::CompileShader(L"Shaders\\13_Blur\\Default.hlsl", Defines, "PS", "ps_5_0");
	Shaders["alphaTestedPS"] = d3dUtil::CompileShader(L"Shaders\\13_Blur\\Default.hlsl", AlphaTestDefines, "PS", "ps_5_0");
	Shaders["horzBlurCS"] = d3dUtil::CompileShader(L"Shaders\\13_Blur\\Blur.hlsl", nullptr, "HorzBlurCS", "cs_5_0");
	Shaders["vertBlurCS"] = d3dUtil::CompileShader(L"Shaders\\13_Blur\\Blur.hlsl", nullptr, "VertBlurCS", "cs_5_0");
...
void FBlur::BuildWavesGeometryBuffers()
{
...
void FBlur::BuildRenderItems()
{
	...
	RenderLayers[(int)URenderLayer::Transparent].push_back(WavesRitem.get());
...
void FBlur::DrawRenderItems(ID3D12GraphicsCommandList *CommandList, const std::vector<FRenderItem *> &RenderItems)
{
...
void FBlur::BuildFrameResources()
{
...
void FBlur::BuildPSOs()
{
	...
	ThrowIfFailed(D3DDevice->CreateGraphicsPipelineState(&alphaTestedPsoDesc, IID_PPV_ARGS(&PSOs["alphaTested"])));
	...
	ThrowIfFailed(D3DDevice->CreateComputePipelineState(&horzBlurPSO, IID_PPV_ARGS(&PSOs["horzBlur"])));
	...
	ThrowIfFailed(D3DDevice->CreateComputePipelineState(&vertBlurPSO, IID_PPV_ARGS(&PSOs["vertBlur"])));
}
